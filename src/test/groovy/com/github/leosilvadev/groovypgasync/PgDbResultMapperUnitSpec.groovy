...
class PgDbResultMapperUnitSpec extends Specification {
  ...
  def "Should map a enum value as String"() {
    def fake = Log.Status.ACTIVE
    given:
    def type = Log.Status.ACTIVE.class
    ...
    then:
    1 * row.getString(column) >> fake

    and:
    result == fake.toString()
  }


  def "Should map null value as null"() {
    given:
    def type = Object
